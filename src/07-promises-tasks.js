...
function willYouMarryMe(isPositiveAnswer) {
  return new Promise((resolve, reject) => {
    let answer = 'Wrong parameter is passed! Ask her again.';
    if (isPositiveAnswer !== '') {
      answer = isPositiveAnswer ? 'Hooray!!! She said "Yes"!' : 'Oh no, she said "No".';
      resolve(answer);
    }
...
function chainPromises(array, action) {
  return new Promise((resolve) => {
    resolve(array.reduce((acc, values) => {
      values.then((num) => acc.push(num)).catch(() => {});
      return acc;
    }, []));
