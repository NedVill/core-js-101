...
function getPowerFunction(exponent) {
  const operand = exponent;
  return function setExpo(x) {
    const num = x;
    return num ** operand;
  };
}
 ...
 *   getPolynom()      => null
 */
function getPolynom(...argts) {
  return function polynom(x) {
    return argts.reduce((acc, item, index) => {
      let summ = acc;
      if (index === argts.length - 1) {
        summ += item;
      } else {
        summ += (item * (x ** (argts.length - 1 - index)));
      }
      return summ;
    }, 0);
  };
